...
# Colores: 0-vacío (blanco), 1-división (azul), 2-crecimiento (rojo)
COLORMAP = {
    0: (1.0, 1.0, 1.0),
    1: (0.2, 0.4, 0.85),
    2: (0.85, 0.2, 0.2),
...
def plot_snapshots(result: ResultadoSimulacion, params: ParametrosCA, n0: int, prob: float, destino: Path) -> None:
    ...
    for ax, t in zip(axes, tiempos):
        ...
        ax.axis('off')
    fig.suptitle(f"Distribución espacial (N0={n0}, P={prob})")
...
def plot_curvas_n0(resultados: Dict[int, ResultadoSimulacion], params: ParametrosCA, destino: Path) -> None:
    ...
    ax.set_ylabel('Concentración microbiana (celdas vivas)')
    ax.set_title('Efecto de la inhibición espacial (N0)')
    ax.legend()
    fig.tight_layout()
    fig.savefig(str(destino), dpi=200)
    plt.close(fig)
...
def plot_curvas_prob(resultados: Dict[float, ResultadoSimulacion], params: ParametrosCA, destino: Path, n0: int) -> None:
    ...
    ax.set_xlabel('Tiempo (h)')
    ax.set_ylabel('Concentración microbiana (celdas vivas)')
    ax.set_title(f'Efecto de la probabilidad de división (N0={n0})')
    ax.legend()
    fig.tight_layout()
    fig.savefig(str(destino), dpi=200)
    plt.close(fig)
...
def plot_sustrato(res: ResultadoSimulacion, params: ParametrosCA, destino: Path, n0: int, prob: float) -> None:
    ...
    ax.set_xlabel('Tiempo (h)')
    ax.set_ylabel('Concentración media de sustrato')
    ax.set_title(f'Evolución del sustrato (N0={n0}, P={prob})')
    fig.tight_layout()
    fig.savefig(str(destino), dpi=200)
    plt.close(fig)
